...
7. Set `plan.status` in the manifest (`tasks.json`) to `"COMPLETED"` using a write-tmp-then-rename pattern when available, only after all required Phase 7 work succeeds, including `plan-executor:pr-finalize` when applicable. On terminal failure paths, set it to `"FAILED"` instead. Do NOT modify the plan markdown.
